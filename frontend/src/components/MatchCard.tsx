...
export default function MatchCard({ item, locale }: MatchCardProps) {
  ...
  const teamLabel = locale === "de" ? "Team:" : "Team:";
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
      <div className="flex gap-4">
        {/* Score */}
        <div className="flex min-w-[68px] flex-col items-center pt-0.5">
          <span className={`text-3xl font-extrabold leading-none ${scoreColor(item.match_score)}`}>
            {item.match_score}
          </span>
          <span className="mt-0.5 text-xs font-medium text-slate-400">/ 100</span>
          <div className="mt-2 h-1.5 w-full rounded-full bg-slate-100">
            <div
              className={`h-1.5 rounded-full ${scoreBgColor(item.match_score)}`}
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-sm text-slate-600">{item.tagline}</p>

          {item.match_begruendung && (
            <p className="mt-2 rounded-r border-l-[3px] border-slate-300 bg-slate-50 px-3 py-1.5 text-sm italic text-slate-500">
              {item.match_begruendung}
            </p>
          )}
